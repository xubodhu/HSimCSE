...
def main():
    ...
    if extension == "txt":
        extension = "text"
    if extension == "csv":
        datasets = load_dataset('/HSimCSE/text.py', data_files=data_files, cache_dir="./data/",
                                delimiter="\t" if "tsv" in data_args.train_file else ",")
    else:
        datasets = load_dataset(
            '/HSimCSE/text.py', data_files=data_files, cache_dir="./data/")
    ...
    def prepare_features(examples):
        ...
        ebd_features = tokenizer(sentences, max_length=data_args.max_seq_length,
                                 truncation=True, padding=True, return_tensors="pt")
        encoded_input = {k: v.to('cuda') for k, v in ebd_features.items()}
        with torch.no_grad():
            model_output = model(**encoded_input, sent_emb=True)
            embds = model_output.last_hidden_state.cpu()
            embds = embds[:, 0]
            embds /= embds.norm(dim=1, keepdim=True)
        ...
        features['origemb1'] = [embds[i] for i in range(total)]
        # features['origemb2'] = [embds[i+total] for i in range(total)]
        return features
    ...
    if training_args.do_train:
        ...
        train_dataset.add_faiss_index(
            column="origemb1", string_factory="Flat", metric_type=faiss.METRIC_INNER_PRODUCT)
